...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET["deptid"])) {
        $dept_id = $_GET["deptid"];
        $dept_stmt = $dbconnect->prepare("SELECT * FROM departments WHERE deptid = :deptid");
        $dept_stmt->bindParam(':deptid', $dept_id);
        $dept_stmt->execute();
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $sql = "UPDATE departments 
                SET deptfullname = :dept_fullname, deptshortname = :dept_shortname, deptcollid = :dept_collid
...
<body>
    <div class="form_container">
        ...
        <form onsubmit="submitForm(event)">
            <div class="form_group">
                ...
                <input type="text" id="dept_id" name="dept_id" style="background: grey;" readonly value="<?php echo $_SESSION['departments']['deptid']; ?>">
            </div>
            <div class="form_group">
                <label for="dept_fullname">Department Full Name</label>
                <input type="text" id="dept_fullname" name="dept_fullname" value="<?php echo $_SESSION['departments']['deptfullname']; ?>">
            </div>
            <div class="form_group">
                <label for="dept_shortname">Department Short Name</label>
                <input type="text" id="dept_shortname" name="dept_shortname" value="<?php echo $_SESSION['departments']['deptshortname']; ?>">
            </div>
            <div class="form_group">
                ...
                <select id="dept_collid" name="dept_collid">
                    ...
                    <?php foreach ($_SESSION['colleges'] as $college): ?>
                        <option value="<?php echo $college['collid']; ?>" <?php if($college['collid'] == $_SESSION['departments']['deptcollid']) echo 'selected'; ?>><?php echo $college['collfullname']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    <script src="./javascript/dept_edit.js"></script>
